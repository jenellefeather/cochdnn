...
def my_collate(batch):
    """Thows out elements that are 'None'"""
    batch = list(filter(lambda x : x[0] is not None, batch))
    return ch.utils.data.dataloader.default_collate(batch)


ds_kwargs = {'transform_train':TRAIN_TRANSFORMS_JSINV3_AUDIO_ONLY, 
             'transform_test':TEST_TRANSFORMS_JSINV3_AUDIO_ONLY}

model, ds = build_network.main(ds_kwargs=ds_kwargs)
...
train_loader, val_loader = ds.make_loaders(batch_size=BATCH_SIZE, 
                                           ...
                                           subset_start_val=0,
                                           dl_kwargs={'collate_fn':my_collate},
                                           subset_val=40650,
                                          )
...
eval_kwargs = {
    'out_dir': "eval_out",
    'exp_name': "eval_natural_clean_jsinv3",
    'adv_train': 0,
    "adv_eval":0, 
    'constraint': '2',
    'eps': 3,
