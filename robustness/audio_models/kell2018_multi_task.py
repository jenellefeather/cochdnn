    ...
    def __init__(self, num_classes=1000):
        ...
        if isinstance(num_classes, dict): # Make multiple fully conected layers
            ...
            for task in num_classes.keys():
                all_fc_layers[task] = nn.Linear(4096, num_classes[task])
    ...
    def forward(self, x, with_latent=False, fake_relu=False, no_relu=False):
        ...
        if isinstance(self.classification, nn.ModuleDict):
            ...
            for task, fc_l in self.classification.items():
                final[task] = fc_l(x_out)
                all_outputs['final/'+task] = final[task]
        else:
            final = self.classification(x_out)
        ...
        if with_latent:
            return final, x_relu, all_outputs
        return final
        
       
def kell2018_multi_task(pretrained=False, progress=True, **kwargs):
